    ...
    fixv_type(nullptr) // jsm
{
  ...
  // jsm
  memory->create(fixv_type, n + 1, "rheo:fixv_type");
  for (i = 1; i <= n; i++) fixv_type[i] = 0; // default: v of all types should be updated 
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "shift") == 0) {
      ...
      iarg += n;
    } else if (strcmp(arg[iarg], "fixed/vel") == 0) { // jsm
      if (iarg + n >= narg) utils::missing_cmd_args(FLERR, "fix rheo fixed/vel", error);
      for (i = 1; i <= n; i++) fixv_type[i] = utils::numeric(FLERR, arg[iarg + i], false, lmp);
      iarg += n;
    } else {
