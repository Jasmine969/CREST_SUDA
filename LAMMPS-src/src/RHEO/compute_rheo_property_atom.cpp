    ...
    compute_surface(nullptr), compute_vshift(nullptr), compute_grad(nullptr)
{
  ...
  for (int iarg = 3; iarg < narg; iarg++) {
    if (strcmp(arg[iarg], "surface/n/*") == 0) {
      ...
      nvalues += dim;
    } else if (strcmp(arg[iarg], "fsph/*") == 0) {
      nvalues += dim;
    } else if (strcmp(arg[iarg], "fbpm/*") == 0) {
      nvalues += dim;
  ...
  for (int iarg = 3; iarg < narg; iarg++) {
    ...
    } else if (utils::strmatch(arg[iarg], "^fsph/")) {
      i += add_vector_component(arg[iarg], i, &ComputeRHEOPropertyAtom::pack_fsph) - 1;
    } else if (utils::strmatch(arg[iarg], "^fbpm/")) {
      i += add_vector_component(arg[iarg], i, &ComputeRHEOPropertyAtom::pack_fbpm) - 1;
...
void ComputeRHEOPropertyAtom::pack_fbpm(int n)
{
  double **fbpm = atom->fbpm;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
  int index = col_index[n];
...
void ComputeRHEOPropertyAtom::pack_fsph(int n)
{
  double **fsph = atom->fsph;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
  int index = col_index[n];
