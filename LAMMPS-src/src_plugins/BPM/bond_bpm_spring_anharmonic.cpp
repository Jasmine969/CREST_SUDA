...
BondBPMSpringAnharmonic::BondBPMSpringAnharmonic(LAMMPS *_lmp) :
    BondBPM(_lmp), k1(nullptr), k2(nullptr), av(nullptr), ecrit(nullptr), gamma(nullptr),
...
BondBPMSpringAnharmonic::~BondBPMSpringAnharmonic()
{
  delete[] svector;
...
double BondBPMSpringAnharmonic::store_bond(int n, int i, int j)
{
...
void BondBPMSpringAnharmonic::store_data()
{
  int i, j, m, type;
  double delx, dely, delz, r;
  double **x = atom->x;
...
void BondBPMSpringAnharmonic::compute(int eflag, int vflag)
{
  ...
  if (volume_flag) {
    ...
    if (nmax < atom->nmax) {
      nmax = atom->nmax;
      memory->create(dvol0, nmax, "bond/bpm/spring/anharmonic:dvol0");
  ...
  // jsm
  double **fbpm = nullptr;
  if (atom->fbpm_flag) fbpm = atom->fbpm;
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    rinv = 1.0 / r;
    if (normalize_flag)
      fbond = -k1[type] * e - k2[type] * e*e;
    else
      fbond = -k1[type] * (r - r0) - k2[type] * (r-r0)*(r-r0); // jsm
    ...
    if (newton_bond || i1 < nlocal) {
      ...
      f[i1][1] += dely * fbond;
      f[i1][2] += delz * fbond;
      if (atom->fbpm_flag) { // jsm
        fbpm[i1][0] += delx * fbond;
        fbpm[i1][1] += dely * fbond;
        fbpm[i1][2] += delz * fbond;
      }
    }
    ...
    if (newton_bond || i2 < nlocal) {
      ...
      f[i2][2] -= delz * fbond;
      if (atom->fbpm_flag) { // jsm
        fbpm[i2][0] -= delx * fbond;
        fbpm[i2][1] -= dely * fbond;
        fbpm[i2][2] -= delz * fbond;
      }
    }
...
int BondBPMSpringAnharmonic::calculate_vol()
{
  int n, i1, i2;
...
void BondBPMSpringAnharmonic::update_vol0()
{
...
void BondBPMSpringAnharmonic::allocate()
{
  allocated = 1;
  const int np1 = atom->nbondtypes + 1;

  memory->create(k1, np1, "bond:k1");
  memory->create(k2, np1, "bond:k2"); // jsm
...
void BondBPMSpringAnharmonic::coeff(int narg, char **arg)
{
  if ((!volume_flag && narg != 5) || (volume_flag && narg != 6))
  ...
  double av_one = 0.0;
  if (volume_flag) av_one = utils::numeric(FLERR, arg[5], false, lmp);
...
void BondBPMSpringAnharmonic::init_style()
{
  BondBPM::init_style();

  if (comm->ghost_velocity == 0)
    error->all(FLERR, "Bond bpm/spring/anharmonic requires ghost atoms store velocity");
...
void BondBPMSpringAnharmonic::settings(int narg, char **arg)
{
...
void BondBPMSpringAnharmonic::write_restart(FILE *fp)
{
...
void BondBPMSpringAnharmonic::read_restart(FILE *fp)
{
...
void BondBPMSpringAnharmonic::write_restart_settings(FILE *fp)
{
...
void BondBPMSpringAnharmonic::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double BondBPMSpringAnharmonic::single(int type, double rsq, int i, int j, double &fforce)
{
  ...
  if (normalize_flag)
    fforce = -k1[type] * e - k2[type] *e*e;
  else
    fforce = -k1[type] * (r - r0) - k2[type] * (r-r0)*(r-r0);
...
int BondBPMSpringAnharmonic::pack_reverse_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
...
void BondBPMSpringAnharmonic::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i, j, m;
  m = 0;
  for (i = 0; i < n; i++) {
...
int BondBPMSpringAnharmonic::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/, int * /*pbc*/)
{
  int i, j, m;
  m = 0;
  for (i = 0; i < n; i++) {
...
void BondBPMSpringAnharmonic::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  m = 0;
  last = first + n;
