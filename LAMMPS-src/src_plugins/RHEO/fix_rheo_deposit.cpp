...
FixRHEODeposit::FixRHEODeposit(LAMMPS *lmp, int narg, char **arg) :
...
{
  if (narg < 7) error->all(FLERR,"Illegal fix rheo/deposit command");
  ...
  if (seed <= 0) error->all(FLERR,"Illegal fix rheo/deposit command");
  ...
  if (mode == ATOM && (ntype <= 0 || ntype > atom->ntypes))
    error->all(FLERR,"Invalid atom type in fix rheo/deposit command");

  // error checks on region and its extent being inside simulation box

  if (!iregion) error->all(FLERR,"Must specify a region in fix rheo/deposit");
  if (iregion->bboxflag == 0)
    error->all(FLERR,"Fix rheo/deposit region does not support a bounding box");
  if (iregion->dynamic_check())
    error->all(FLERR,"Fix rheo/deposit region cannot be dynamic");
  ...
  if (mode == MOLECULE) {
    for (int i = 0; i < nmol; i++) {
      if (onemols[i]->xflag == 0)
        error->all(FLERR,"Fix rheo/deposit molecule must have coordinates");
      if (onemols[i]->typeflag == 0)
        error->all(FLERR,"Fix rheo/deposit molecule must have atom types");
      if (ntype+onemols[i]->ntypes <= 0 ||
          ntype+onemols[i]->ntypes > atom->ntypes)
        error->all(FLERR,"Invalid atom type in fix rheo/deposit mol command");

      if (atom->molecular == Atom::TEMPLATE && onemols != atom->avec->onemols)
        error->all(FLERR,"Fix rheo/deposit molecule template ID must be same "
                   "as atom_style template ID");
      onemols[i]->check_attributes();

      // fix rheo/deposit uses geoemetric center of molecule for insertion
  ...
  if (rigidflag && mode == ATOM)
    error->all(FLERR,"Cannot use fix rheo/deposit rigid and not molecule");
  if (shakeflag && mode == ATOM)
    error->all(FLERR,"Cannot use fix rheo/deposit shake and not molecule");
  if (rigidflag && shakeflag)
    error->all(FLERR,"Cannot use fix rheo/deposit rigid and shake");
...
void FixRHEODeposit::init()
{
  ...
  if (!iregion) error->all(FLERR,"Region ID {} for fix rheo/deposit does not exist", idregion);
  ...
  if (rigidflag) {
    ...
    if (!fixrigid) error->all(FLERR,"Fix rheo/deposit rigid fix ID {} does not exist", idrigid);
    int tmp;
    if (onemols != (Molecule **) fixrigid->extract("onemol",tmp))
      error->all(FLERR, "Fix rheo/deposit and rigid fix are not using the same molecule template ID");
  }
  ...
  if (shakeflag) {
    ...
    if (!fixshake) error->all(FLERR,"Fix rheo/deposit shake fix ID {} does not exist", idshake);
    int tmp;
    if (onemols != (Molecule **) fixshake->extract("onemol",tmp))
      error->all(FLERR,"Fix rheo/deposit and fix shake are not using the same molecule template ID");
  }
  ...
  if (atom->radius_flag) {
    ...
    if (sqrt(nearsq) < separation && comm->me == 0)
      error->warning(FLERR,"Fix rheo/deposit near setting < possible overlap separation {}",separation);
  }
}

/* ---------------------------------------------------------------------- */

void FixRHEODeposit::setup_pre_exchange()
{
...
void FixRHEODeposit::pre_exchange()
{
  ...
  while (attempt < maxattempt) {
    ...
    } else error->all(FLERR,"Unknown particle distribution in fix rheo/deposit");
    ...
    for (m = 0; m < natom; m++) {
      ...
      if (flag) {
        ...
        atom->v[n][2] = vnew[2];
        atom->rho[n] = rho0; // jsm
        atom->rheo_status[n] = status0; // jsm
        if (mode == MOLECULE) {
...
void FixRHEODeposit::find_maxid()
{
  tagint *tag = atom->tag;
...
void FixRHEODeposit::options(int narg, char **arg)
{
  // defaults
  ...
  rz = 0.0;
  rho0 = 1000.0; // jsm
  status0 = 0; // fluid is default

  int iarg = 0;
  while (iarg < narg) {
    if (strcmp(arg[iarg],"region") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix rheo/deposit command");
      iregion = domain->get_region_by_id(arg[iarg+1]);
      if (!iregion) error->all(FLERR,"Region ID {} for fix rheo/deposit does not exist",arg[iarg+1]);
      idregion = utils::strdup(arg[iarg+1]);
      iarg += 2;

    } else if (strcmp(arg[iarg], "var") == 0) {
      if (iarg + 2 > narg) utils::missing_cmd_args(FLERR, "fix rheo/deposit var", error);
      delete[] vstr;
      vstr = utils::strdup(arg[iarg + 1]);
      varflag = 1;
      iarg += 2;
    } else if (strcmp(arg[iarg], "set") == 0) {
      if (iarg + 3 > narg) utils::missing_cmd_args(FLERR, "fix rheo/deposit set", error);
      if (strcmp(arg[iarg + 1], "x") == 0) {
        ...
        error->all(FLERR, "Unknown fix rheo/deposit set option {}", arg[iarg + 2]);
      iarg += 3;

    } else if (strcmp(arg[iarg],"mol") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix rheo/deposit command");
      int imol = atom->find_molecule(arg[iarg+1]);
      if (imol == -1) error->all(FLERR,"Molecule template ID for fix rheo/deposit does not exist");
      mode = MOLECULE;
      ...
      if (mode != MOLECULE) error->all(FLERR,"Illegal fix rheo/deposit command");
      if (iarg+nmol+1 > narg) error->all(FLERR,"Illegal fix rheo/deposit command");
      ...
      if (molfrac[nmol-1] < 1.0-EPSILON || molfrac[nmol-1] > 1.0+EPSILON)
        error->all(FLERR,"Illegal fix rheo/deposit command");
      molfrac[nmol-1] = 1.0;
      iarg += nmol+1;
      ...
      else error->all(FLERR,"Illegal fix rheo/deposit command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"global") == 0) {
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix rheo/deposit command");
      globalflag = 1;
      localflag = 0;
      ...
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix rheo/deposit command");
      localflag = 1;
      globalflag = 0;
      lo = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      hi = utils::numeric(FLERR,arg[iarg+2],false,lmp);
      ...
      if (domain->dimension == 2 && (rx != 0.0 || ry != 0.0))
        error->all(FLERR,"Illegal fix rheo/deposit orient settings");
      ...
      if (iarg+5 > narg) error->all(FLERR,"Illegal fix rheo/deposit command");
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix rheo/deposit command");
      rho0 = utils::numeric(FLERR,arg[iarg+1],false,lmp); // jsm
      iarg += 2;
    } else if (strcmp(arg[iarg],"status") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix rheo/deposit command");
      status0 = utils::numeric(FLERR,arg[iarg+1],false,lmp); // jsm
      iarg += 2;
    } else error->all(FLERR,"Illegal fix rheo/deposit command");
  }
  ...
  if (!vstr && (xstr || ystr || zstr))
    error->all(FLERR, "Incomplete use of variables in fix rheo/deposit command");
  if (vstr && (!xstr && !ystr && !zstr))
    error->all(FLERR, "Incomplete use of variables in fix rheo/deposit command");

  if (varflag) {
    vvar = input->variable->find(vstr);
    if (vvar < 0) error->all(FLERR, "Variable {} for fix rheo/deposit does not exist", vstr);
    if (!input->variable->equalstyle(vvar))
      error->all(FLERR, "Variable for fix rheo/deposit is invalid style");

    if (xstr) {
      xvar = input->variable->find(xstr);
      if (xvar < 0) error->all(FLERR, "Variable {} for fix rheo/deposit does not exist", xstr);
      if (!input->variable->internalstyle(xvar))
        error->all(FLERR, "Variable for fix rheo/deposit is invalid style");
    }
    if (ystr) {
      yvar = input->variable->find(ystr);
      if (yvar < 0) error->all(FLERR, "Variable {} for fix rheo/deposit does not exist", ystr);
      if (!input->variable->internalstyle(yvar))
        error->all(FLERR, "Variable for fix rheo/deposit is invalid style");
    }
    if (zstr) {
      zvar = input->variable->find(zstr);
      if (zvar < 0) error->all(FLERR, "Variable {} for fix rheo/deposit does not exist", zstr);
...
double FixRHEODeposit::compute_scalar()
{
  return ninserted;
}
...
void FixRHEODeposit::write_restart(FILE *fp)
{
  int n = 0;
  double list[5];
  list[n++] = random->state();
...
void FixRHEODeposit::restart(char *buf)
{
  int n = 0;
...
void *FixRHEODeposit::extract(const char *str, int &itype)
{
...
int FixRHEODeposit::vartest(double x, double y, double z)
{
