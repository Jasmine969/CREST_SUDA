...
PairSPHRhoSumNorm::PairSPHRhoSumNorm(LAMMPS *lmp) : Pair(lmp)
{
  if (atom->rho_flag != 1)
    error->all(FLERR, Error::NOLASTLINE,
               "Pair sph/rhosum/norm requires atom attribute density, e.g. in atom_style sph");
...
PairSPHRhoSumNorm::~PairSPHRhoSumNorm()
{
  if (allocated) {
...
void PairSPHRhoSumNorm::init_style()
{
...
void PairSPHRhoSumNorm::compute(int eflag, int vflag)
{
      ...
      for (ii = 0; ii < inum; ii++) {
        ...
        for (jj = 0; jj < jnum; jj++) {
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            // jsm
            numrt[ii] += mass[jtype] * wf;
            denom[ii] += mass[jtype] / rho[j] * wf;
            ++coord[ii];
          }
        }
      }
...
void PairSPHRhoSumNorm::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHRhoSumNorm::coeff(int narg, char **arg)
{
  if (narg < 3)
    error->all(FLERR,"Incorrect number of args for sph/rhosum/norm coefficients" + utils::errorurl(21));
  if (!allocated)
    allocate();
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "zmin/rho0") == 0) {
      if (iarg + 3 > narg) utils::missing_cmd_args(FLERR, "pair sph/rhosum/norm", error);
...
double PairSPHRhoSumNorm::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
    error->all(FLERR, Error::NOLASTLINE,
               "All pair sph/rhosum/norm coeffs are not set. Status:\n"
...
double PairSPHRhoSumNorm::single(int /*i*/, int /*j*/, int /*itype*/, int /*jtype*/, double /*rsq*/,
...
int PairSPHRhoSumNorm::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHRhoSumNorm::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
  double *rho = atom->rho;
