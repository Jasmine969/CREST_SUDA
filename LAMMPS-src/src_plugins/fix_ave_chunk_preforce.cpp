...
FixAveChunkPreforce::FixAveChunkPreforce(LAMMPS *lmp, int narg, char **arg) :
...
{
  if (narg < 7) utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce", error);
  ...
  while (iarg < nargnew) {
    ...
    if (strcmp(arg[iarg],"vx") == 0) {
      ...
      if ((argi.get_type() == ArgInfo::UNKNOWN) || (argi.get_dim() > 1))
        error->all(FLERR, val.iarg, "Unknown fix ave/chunk/preforce data value: {}", arg[iarg]);
  ...
  if (nvalues == 0)
    error->all(FLERR, ioffset,
               "No values from computes, fixes, or variables used in fix ave/chunk/preforce command");
  ...
  while (iarg < nargnew) {
    if (strcmp(arg[iarg],"norm") == 0) {
      if (iarg+2 > nargnew) utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce norm", error);
      if (strcmp(arg[iarg+1],"all") == 0) {
        ...
        normflag = SAMPLE;
        scaleflag = NOSCALE;
      } else error->all(FLERR, iarg + 1, "Unknown fix ave/chunk/preforce norm mode: {}", arg[iarg+1]);
      iarg += 2;
    } else if (strcmp(arg[iarg],"ave") == 0) {
      if (iarg+2 > nargnew) utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce ave", error);
      if (strcmp(arg[iarg+1],"one") == 0) ave = ONE;
      else if (strcmp(arg[iarg+1],"running") == 0) ave = RUNNING;
      else if (strcmp(arg[iarg+1],"window") == 0) ave = WINDOW;
      else error->all(FLERR, iarg + 1, "Unknown fix ave/chunk/preforce ave mode: {}", arg[iarg+1]);
      if (ave == WINDOW) {
        if (iarg+3 > nargnew) utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce ave window", error);
        nwindow = utils::inumeric(FLERR,arg[iarg+2],false,lmp);
        if (nwindow <= 0)
          error->all(FLERR, iarg + 2, "Illegal fix ave/chunk/preforce number of windows: {}", nwindow);
      }
      iarg += 2;
      if (ave == WINDOW) iarg++;

    } else if (strcmp(arg[iarg],"bias") == 0) {
      if (iarg+2 > nargnew) utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce bias", error);
      biasflag = 1;
      id_bias = utils::strdup(arg[iarg+1]);
      iarg += 2;
    } else if (strcmp(arg[iarg],"adof") == 0) {
      if (iarg+2 > nargnew) utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce adof", error);
      adof = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      iarg += 2;
    } else if (strcmp(arg[iarg],"cdof") == 0) {
      if (iarg+2 > nargnew) utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce cdof", error);
      cdof = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      iarg += 2;

    } else if ((strcmp(arg[iarg],"file") == 0) || (strcmp(arg[iarg],"append") == 0)) {
      if (iarg+2 > nargnew)
        utils::missing_cmd_args(FLERR, std::string("fix ave/chunk/preforce ")+arg[iarg], error);
      if (comm->me == 0) {
        if (strcmp(arg[iarg],"file") == 0) fp = fopen(arg[iarg+1],"w");
        else fp = fopen(arg[iarg+1],"a");
        if (fp == nullptr)
          error->one(FLERR, iarg+1, "Cannot open fix ave/chunk/preforce file {}: {}", arg[iarg+1],
                     utils::getsyserror());
      }
      iarg += 2;
      ...
      if (iarg+2 > nargnew)  utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce format", error);
      delete[] format;
      format = utils::strdup(arg[iarg+1]);
      iarg += 2;
    } else if (strcmp(arg[iarg],"title1") == 0) {
      if (iarg+2 > nargnew)  utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce title1", error);
      delete[] title1;
      title1 = utils::strdup(arg[iarg+1]);
      iarg += 2;
    } else if (strcmp(arg[iarg],"title2") == 0) {
      if (iarg+2 > nargnew)  utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce title2", error);
      delete[] title2;
      title2 = utils::strdup(arg[iarg+1]);
      iarg += 2;
    } else if (strcmp(arg[iarg],"title3") == 0) {
      if (iarg+2 > nargnew)  utils::missing_cmd_args(FLERR, "fix ave/chunk/preforce title3", error);
      delete[] title3;
      title3 = utils::strdup(arg[iarg+1]);
      iarg += 2;
    } else error->all(FLERR, iarg, "Unknown fix ave/chunk/preforce keyword: {}", arg[iarg]);
  }

  // setup and error check

  if (nevery <= 0) error->all(FLERR, 3, "Illegal fix ave/chunk/preforce nevery value: {}", nevery);
  if (nrepeat <= 0) error->all(FLERR, 4, "Illegal fix ave/chunk/preforce nrepeat value: {}", nrepeat);
  if (nfreq <= 0) error->all(FLERR, 5, "Illegal fix ave/chunk/preforce nfreq value: {}", nfreq);
  if (nfreq % nevery || nrepeat*nevery > nfreq)
    error->all(FLERR, Error::NOPOINTER, "Inconsistent fix ave/chunk/preforce nevery/nrepeat/nfreq values");
  ...
  for (auto &val : values) {
    ...
    if (val.which == ArgInfo::COMPUTE) {
      ...
      if (!val.val.c)
        error->all(FLERR, val.iarg, "Compute ID {} for fix ave/chunk/preforce does not exist",val.id);
      ...
      if (!val.val.f)
        error->all(FLERR, val.iarg, "Fix ID {} for fix ave/chunk/preforce does not exist",val.id);
      ...
      if (val.val.v < 0)
        error->all(FLERR, val.iarg, "Variable name {} for fix ave/chunk/preforce does not exist", val.id);
  ...
  if (!cchunk)
    error->all(FLERR, 6, "Chunk/atom compute {} does not exist or is incorrect style for "
               "fix ave/chunk/preforce", idchunk);
  ...
  // nvalid = next step on which pre_force does something
  // add nvalid to all computes that store invocation times
  // since don't know a priori which are invoked by this fix
  // once in pre_force() can set timestep for ones actually invoked
...
FixAveChunkPreforce::~FixAveChunkPreforce()
{
...
int FixAveChunkPreforce::setmask()
{
  int mask = 0;
  mask |= PRE_FORCE;
  return mask;
...
void FixAveChunkPreforce::init()
{
  ...
  if (!cchunk)
    error->all(FLERR, Error::NOLASTLINE, "Chunk/atom compute {} does not exist or is "
               "incorrect style for fix ave/chunk/preforce",idchunk);
  ...
  for (auto &val : values) {
    if (val.which == ArgInfo::COMPUTE) {
      val.val.c = modify->get_compute_by_id(val.id);
      if (!val.val.c)
        error->all(FLERR, Error::NOLASTLINE, "Compute ID {} for fix ave/chunk/preforce does not exist", val.id);

    } else if (val.which == ArgInfo::FIX) {
      val.val.f = modify->get_fix_by_id(val.id);
      if (!val.val.f)
        error->all(FLERR, Error::NOLASTLINE, "Fix ID {} for fix ave/chunk/preforce does not exist", val.id);

      if (nevery % val.val.f->peratom_freq)
        error->all(FLERR, Error::NOLASTLINE, "Fix {} for fix ave/chunk/preforce not computed at "
                   "compatible time{}", val.id, utils::errorurl(7));

    } else if (val.which == ArgInfo::VARIABLE) {
      val.val.v = input->variable->find(val.id.c_str());
      if (val.val.v < 0)
        error->all(FLERR, Error::NOLASTLINE, "Variable name {} for fix ave/chunk/preforce does not exist"
                   , val.id);
    }
  }
...
/* ----------------------------------------------------------------------
   ...
   do not call setup_chunks(), even though fix ave/chunk/preforce called setup_bins()
...
void FixAveChunkPreforce::setup_pre_force(int vflag)
{
  pre_force(vflag);
}
...
void FixAveChunkPreforce::pre_force(int /*vflag*/)
{
  int i,j,m,index;
...
void FixAveChunkPreforce::allocate()
{
...
double FixAveChunkPreforce::compute_array(int i, int j)
{
...
/* ----------------------------------------------------------------------
   calculate nvalid = next step on which pre_force does something
   can be this timestep if multiple of nfreq and nrepeat = 1
   else backup from next multiple of nfreq
------------------------------------------------------------------------- */

bigint FixAveChunkPreforce::nextvalid()
{
...
double FixAveChunkPreforce::memory_usage()
{
