...
sys.path.append('/data/zhuhong_codes/EntericNervousSystem')
...
""")
if 'kA' in p_global:
    lmp.command(f"variable kA equal {p_global['kA']}")
...
def callback(caller, step, nlocal, tag, x, fext):
    global net_comp_time, interact_comp_time
    ...
    if me == 0:
        ...
        yz_all = np.asarray(xyz_all).reshape(-1, n_yz, 3)[:, :, 1:]  # (200,63,2)
        ...
        if mylog_flag:
            ...
            with open(f'{case_path}/my{go_on_from_step}_to{end_step}.log', 'a') as f:
                print(f'{step_info}\n{value_info}\n{time_info}\n\n', file=f)
    if strain_flag:
        warn('The strain is too negative!')
        caller.error(LMP_ERROR_ALL, 'The strain is too negative!')
