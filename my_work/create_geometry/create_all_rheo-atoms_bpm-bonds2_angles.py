...
if r_catheter == r_si:
    lid_in = gm.Geometry()
    pipe_in_hrz = gm.CylinderSide(
        r=r_si, l_axis=l_pipe_hrz - dl, dl=dl, axis='x',
    ).shift(x=-l_pipe_hrz)
else:
    lid_in = gm.ThickRing(r_out=r_si, r_in=r_catheter, dl=dl,
...
print(log_n_ring)
inlet = gm.Union((pipe_in_vert, torus_in, lid_in, pipe_in_hrz))
si = gm.CylinderSide(r=r_si, l_axis=l_si, dl=dl, axis='x')
...
fluid1 = gm.Stack(
    gm.ThickRing(r_out=r_fluid1, r_in=0, dl=dl, incl_inner=True, incl_outer=False, axis='x'),
    'x', n_axis_fluid1, dl
).shift(x=-l_pipe_hrz + dl)
r_fluid2 = r_catheter
...
fluid2 = gm.Stack(
    gm.ThickRing(r_out=r_fluid2, r_in=0, dl=dl, incl_inner=True, incl_outer=False, axis='z'),
    'z', n_axis_fluid2, dl
...
get_wall_ID = partial(gm.get_wall_ID, n_per_ring=n_ring_si, smallest_ID=n_atoms_inlet + 1)
...
filename = 'equip_si-rheo_'

filename += f'bond2_angle_dl{dl:.1e}-semifilled'
lmp.commands_string(f"""
run             0
